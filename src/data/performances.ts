 ...
 *
 * **Array order is the running order.** `year` is derived from the file's
 * timestamp, which for footage that has been through Drive is the date of the
 * export and not of the performance — `scripts/ingest-video.mjs` says so where
 * it guesses. Where the two disagree the order here is the one that was
 * corrected by hand, so anything showing these as a sequence (the music shelf)
 * reads this array top to bottom rather than sorting on `year`.
 *
 * TODO(sanjana): replace the descriptive titles with the real programme names,
 * add venue / city / repertoire per entry, and correct `year` to the real
 * performance dates — at which point the order and the years agree again.
 */
...
/**
 * The still a piece shows before it plays.
 *
 * Every one of these was chosen by hand out of its own footage; none is
 * sampled. The generation is in the filename for the same reason `preview`
 * carries its cut size — the bucket sends an immutable one-year cache
 * header, so a replacement that arrives under the key the old one occupied
 * stays invisible, in browsers and at the edge alike, until that year is up.
 * A re-grab lands on `-v3`, never back on this key.
 *
 * They are cut to 16:9, the shape `WorkFrame` gives every tile, so the grid
 * shows the frame that was picked rather than a crop of it. That is
 * deliberately not the shape of the film — a good half of this archive is
 * phone footage at 9:16 — and `aspect` on each entry still records the
 * film's own ratio, which is what the player and the gallery size from.
 */
const poster = (slug: string) => `/media/posters/${slug}-v2.jpg`
...
export const PERFORMANCES: Performance[] = [
  ...
  /* ---------------- Duet/Group Performances ----------------
     ...
     `Trio, Red Curtain` leads deliberately: it is the earliest of these,
     whatever its export stamp says. The rest keep the order they were
     ingested in, which is the best guess available until the real dates
     are known. */
  {
  ...
  /* ---------------- Solo Concerts ----------------
     ...
     Running order, not slug order. `year` here is an export stamp rather
     than a recording date (see the header note), and for these twelve it
     runs almost exactly backwards: the last file exported is the earliest
     performance. So they are listed the way they were performed, which
     means the years below descend. Trust the order, not the number.
