 ...
 */
export function Overture({ src, poster, label, children, length = 3.2 }: Props) {
  ...
  const width = useTransform(open, [0, 1], ['30vw', '100vw'])
  const height = useTransform(open, [0, 1], ['26vh', '100vh'])
  ...
  const filmY = useTransform(open, [0, 1], ['28vh', '0vh'])
  ...
  const copyY = useTransform(open, [0, 1], ['-10vh', '-82vh'])
    ...
    return (
      <section className="relative bg-void px-6 pt-32 pb-16 md:px-12">
        <div className="mx-auto max-w-[1500px] text-center">
          {label && <p className="label text-bloom">{label}</p>}
  ...
  return (
    ...
    >
      <div className="sticky top-0 h-screen overflow-hidden">
        ...
        >
          <div className="pointer-events-auto mx-auto max-w-[1500px] text-center">
            {label && <p className="label mb-8 text-bloom">{label}</p>}
            {children}
          </div>
        </motion.div>
      </div>
